...
public class OrderServiceImpl implements OrderService {
    ...
    public OrderSubmitVO submitOrder(OrdersSubmitDTO ordersSubmitDTO) {
        ...
        // 1. 异常处理（包括地址簿为空、购物车为空，提高代码健壮性，以及配送地址不可达）
    ...
    /**
     * 再来一单
     * 业务实现就是将选购的菜品都添加到购物车中，用户后续自行手动结算
     */
    @Override
    public void repetition(Long id) {
        ...
        // 将订单详情对象转换为购物车对象 - stream 高效对集合/数组操作
        List<ShoppingCart> shoppingCartList = orderDetailList.stream().map(x -> {
            ShoppingCart shoppingCart = new ShoppingCart();

            // 将原订单详情里面的每个菜品信息，重新复制到每个购物车对象中
    ...
    public void remainder(Long id) {
        ...
        // 推送催单消息
        Map<String, Object> message = new HashMap<>();
        message.put("type", 2);     // 1-来单提醒，2-客户催单
    ...
    public PageResult conditionSearch(OrdersPageQueryDTO ordersPageQueryDTO) {
        ...
        // 部分订单状态，需要额外返回订单菜品信息，将 Orders 封装为 OrderVO
        ...
        if (payStatus == Orders.PAID) {
            ...
            if (false) {
                String refund = weChatPayUtil.refund(
                        ...
                        new BigDecimal("0.01"),
                        new BigDecimal("0.01"));
                log.info("[Service] 申请退款：{}", refund);
            }
        }

        // 更新订单状态 (根据订单 id 更新订单状态、拒单原因、取消时间)
        Orders orders = new Orders();
        orders.setId(ordersDB.getId());
        orders.setStatus(Orders.CANCELLED);
    ...
    public void cancel(OrdersCancelDTO ordersCancelDTO) throws Exception {
        ...
        // 更新订单状态 (根据订单 id 更新订单状态、取消订单原因、取消时间)
        Orders orders = new Orders();
    ...
    public void complete(Long id) {
        ...
        // 校验订单是否存在，并且状态为 4 - DELIVERY_IN_PROGRESS
    ...
    /**
     * 根据订单数据 (订单id) 获取菜品信息字符串
     * @param orders 订单数据 (订单id)
     * @return 菜品信息字符串
     */
